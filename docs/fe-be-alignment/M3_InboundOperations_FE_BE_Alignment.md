...
**FE Pages:** `frontend/src/pages/inbound-operations/` (5 pages + 1 layout)
...
## 7. FE Pages mới phát hiện (CẬP NHẬT 2026-03-11)

| FE Page | Exported | Route | Ngôn ngữ | Ghi chú |
|---------|----------|-------|----------|---------|
| `PurchaseOrdersPage` | ✅ | ❌ Chưa có route | Hỗn hợp (title EN, buttons/modals VN) | CRUD PO, Confirm, Cancel, Close |
| `InboundOperationsLayout` | ✅ | ✅ Layout wrapper | English | Nav đã có "Purchase Orders" item |

> ⚠️ `PurchaseOrdersPage` có đầy đủ CRUD nhưng **chưa có route** và **chưa có BE endpoints** cho PO entity.

---

## 8. Trạng thái tổng thể Module 3 (Inbound) — CẬP NHẬT 2026-03-11

| Mục | Điểm |
|-----|------|
| FE Pages | 5 pages + 1 layout (tăng từ 4: +PurchaseOrdersPage) |
...
| Module registration | ❌ Chưa đăng ký vào NestJS app (`app.module.ts` chỉ có Foundation, MasterData, Auth) |
| Missing routes | 1 (PurchaseOrdersPage) |
| **Tổng** | **🔴 BLOCKED — 2 system blockers + 5 criticals + 1 missing route** |
