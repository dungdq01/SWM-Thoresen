...
**FE Pages:** `frontend/src/pages/outbound-operations/` (5 pages + 1 layout)
...
## 6. FE Pages mới phát hiện (CẬP NHẬT 2026-03-11)

| FE Page | Exported | Route | Ngôn ngữ | Ghi chú |
|---------|----------|-------|----------|---------|
| `SalesOrdersPage` | ✅ | ❌ Chưa có route | Hỗn hợp (title EN, buttons/modals VN) | CRUD SO, Confirm, Cancel, Close, Lines |
| `OutboundOperationsLayout` | ✅ | ✅ Layout wrapper | English | Nav đã có "Sales Orders" item |

> ⚠️ `SalesOrdersPage` có đầy đủ CRUD nhưng **chưa có route** và **chưa có BE endpoints** cho SO entity.

---

## 7. Trạng thái tổng thể Module 6 (Outbound) — CẬP NHẬT 2026-03-11

| Mục | Điểm |
|-----|------|
| FE Pages | 5 pages + 1 layout (tăng từ 4: +SalesOrdersPage) |
| API coverage | **100%** (18/18 FE calls match BE routes — cho shipment flow) |
...
| Missing routes | 1 (SalesOrdersPage) |
| **Tổng** | **⚠️ CONDITIONAL PASS — Cần: (1) import module, (2) fix double-prefix, (3) thêm route SalesOrdersPage** |
