  ...
  return (
    <div className="page-section">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.4fr_1fr]">
        ...
        <div className="wrs-card p-5 space-y-4">
          <div>
            <h3 className="text-sm font-semibold text-navy-900">Create VAS Work Order</h3>
            <p className="text-sm text-navy-400">Create bagging or repacking work order.</p>
          </div>
          ...
          <div>
            <Select label="Source Item" value={draft.sourceItemId} onChange={(e) => setDraft((prev) => ({ ...prev, sourceItemId: e.target.value }))} options={items.map((i) => ({ value: i.id, label: `${i.code} - ${i.name}` }))} />
          ...
          <Button variant="accent" onClick={handleCreate} disabled={createWorkOrder.isPending}>Create Work Order</Button>
        </div>
      </div>
    </div>
  )
