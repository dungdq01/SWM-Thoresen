    ...
    <div className="page-section">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.3fr_1fr]">
                ...
                <TableRow key={row.id} onClick={() => setSelectedId(row.id)} className={selected?.id === row.id ? 'bg-muted/60' : ''}>
                  ...
                  <TableCell align="center">
                    {row.status === 'ALL_WEIGHED' ? (
                      <Button variant="accent" size="sm" onClick={() => shipShipment.mutate(row.id)}>Ship</Button>
                    ) : (
        ...
        <div className="space-y-5">
          ...
          <div className="wrs-card p-5 space-y-4">
            <h3 className="text-sm font-semibold text-navy-900">Record Tare (empty vehicle)</h3>
            ...
            <Button variant="accent" onClick={handleTare} disabled={!selected || selected.status !== 'PICKED' || recordTare.isPending}>Record Tare</Button>
          </div>

          <div className="wrs-card p-5 space-y-4">
            <h3 className="text-sm font-semibold text-navy-900">Record Gross (per line)</h3>
            ...
            <Button variant="accent" onClick={handleGross} disabled={!selected || !selected.tareWeightKg || recordGross.isPending}>Record Gross</Button>
          </div>

          <div className="wrs-card p-5 space-y-4">
            <h3 className="text-sm font-semibold text-navy-900">Lines & Tolerance</h3>
          ...
          <div className="wrs-card p-5 space-y-3">
            <h3 className="text-sm font-semibold text-navy-900">Weighing History</h3>
            <div className="space-y-2 text-sm text-navy-700 max-h-48 overflow-y-auto">
              {weighingHistory.length === 0 ? (
                <p className="text-navy-400">No weighing history yet</p>
              ) : (
