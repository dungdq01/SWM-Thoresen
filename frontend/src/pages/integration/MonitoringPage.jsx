    ...
    <>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mb-5">
        <div className="wrs-card p-5">
          ...
          <SummaryDonut
            centerLabel="Kênh"
            data={[
              { name: 'Hoạt động tốt', value: overview.healthyChannels || 0, color: '#059669' },
        ...
        <div className="wrs-card p-5 flex flex-col gap-3">
          <h3 className="text-sm font-semibold text-navy-900">Cân xe tải</h3>
          <StatHighlight value={stats.weighbridgeStats?.totalEvents || 0} label="Tổng sự kiện" color="text-navy-900" bgColor="bg-moon-50" />
          <StatHighlight value={`${stats.weighbridgeStats?.avgProcessingMs || 0}ms`} label="TB xử lý" color="text-blue-600" bgColor="bg-blue-50" />
        </div>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
            ...
            {channels.map((channel) => (
              <div key={channel.id} className="flex items-center justify-between border-b border-moon-200 pb-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-navy-400 mt-1">Thời gian hoạt động: {channel.uptimePercent}%</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-5">
          ...
          <div className="wrs-card p-5 space-y-3">
            <h3 className="text-sm font-semibold text-navy-900">Thống kê đồng bộ di động</h3>
            <div className="grid grid-cols-2 gap-3">
              <StatHighlight value={stats.mobileSyncStats?.totalSyncs || 0} label="Tổng đồng bộ" color="text-navy-900" bgColor="bg-moon-50" />
              <StatHighlight value={stats.mobileSyncStats?.pendingItems || 0} label="Đang chờ" color="text-amber-600" bgColor="bg-amber-50" />
            </div>
          </div>
        </div>
      </div>
