  ...
  return (
    <div className="page-section">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.4fr_1fr]">
        ...
        <div className="wrs-card p-5 space-y-4">
          <div>
            <h3 className="text-sm font-semibold text-navy-900">Create Transfer Order</h3>
            <p className="text-sm text-navy-400">Transfer inventory between warehouses with IN_TRANSIT status.</p>
          </div>
          <div>
          ...
          <div className="border-t border-moon-200 pt-4">
            ...
            <Select label="Owner" value={draft.lines[0].ownerId} onChange={(e) => updateLine(0, 'ownerId', e.target.value)} options={[{ value: '', label: '-- Chọn Owner --' }, ...owners.map((o) => ({ value: o.id, label: `${o.code} - ${o.name}` }))]} className="mt-3" />
            <Select label="From Location" value={draft.lines[0].fromLocationId} onChange={(e) => updateLine(0, 'fromLocationId', e.target.value)} options={[{ value: '', label: '-- Chọn Location --' }, ...locations.map((l) => ({ value: l.id, label: l.code }))]} className="mt-3" />
            <div className="mt-3">
          ...
          <Button variant="accent" onClick={handleCreate} disabled={createTransferOrder.isPending}>Create Transfer Order</Button>
        </div>
      </div>
    </div>
  )
