...
export function TransferOrdersPage() {
  ...
  const validate = () => {
    const e = {}
    if (!draft.fromWarehouseId) e.fromWarehouseId = 'Kho nguồn là bắt buộc'
    if (!draft.toWarehouseId) e.toWarehouseId = 'Kho đích là bắt buộc'
    if (draft.fromWarehouseId && draft.toWarehouseId && draft.fromWarehouseId === draft.toWarehouseId) e.toWarehouseId = 'Kho nguồn và đích không được giống nhau'
  ...
  return (
    <div className="page-section">
      <div className="page-header">
        <h2 className="section-title">Lệnh chuyển kho liên kho</h2>
              ...
              <TableRow key={row.id}>
                ...
                <TableCell align="center">
                  <div className="flex justify-center gap-2">
                    {row.status === 'CREATED' && <Button variant="outline" size="sm" onClick={() => releaseTransferOrder.mutate(row.id)}>Phát hành</Button>}
                    {row.status === 'RELEASED' && <Button variant="accent" size="sm" onClick={() => shipTransferOrder.mutate({ id: row.id, data: {} })}>Xuất</Button>}
                    {row.status === 'IN_TRANSIT' && <Button variant="accent" size="sm" onClick={() => receiveTransferOrder.mutate({ id: row.id, data: {} })}>Nhận</Button>}
      ...
      <Modal
        ...
        description="Chuyển hàng tồn giữa các kho với trạng thái đang vận chuyển."
        size="lg"
        footer={
          <>
            ...
            <Button variant="accent" onClick={handleCreate} disabled={createTransferOrder.isPending}>
              {createTransferOrder.isPending ? 'Đang xử lý...' : 'Tạo lệnh chuyển kho'}
            </Button>
          </>
        }
      >
